...
class SingleResultsProcessor:
    ...
    def plot_IV(self, label, err=True, alternative=False, errorevery=10,
        ...
        if err:
            ...
            ax.plot(self.V[:self.mid_idx]/Vnorm, self.I[:self.mid_idx]/Inorm + shift, fmt_up , label=label + " up")
            ax.plot(self.V[self.mid_idx:]/Vnorm, self.I[self.mid_idx:]/Inorm + shift, fmt_down , label=label + " down")
        if alternative:
