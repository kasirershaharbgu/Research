...
qsub -V -S /bin/bash -cwd -N one_dimension_array_r_disorder -q 
./run_RC_SET.sh 1 "$N" "$vmin" "$vmax" "$vstep" "$tstep" "$vg_avg" 0 "$c_avg" 0 "$cg_avg" 0 "$r_avg" 0  "$rg_avg" 0 "$repeats" "$n_avg" "$n_std" "$q_avg" "$q_std" "$dt" no_disorder_N_"$N" "$f"
